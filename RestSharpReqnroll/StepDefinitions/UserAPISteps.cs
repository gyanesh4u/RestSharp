...
namespace RestSharpReqnroll.StepDefinitions
{
    ...
    public class UserAPISteps
    {
        ...
        // ✅ Inject ScenarioContext directly (Reqnroll discovery FIX)
        public UserAPISteps(ScenarioContext scenarioContext)
        {
            _scenarioContext = new ScenarioContextHelper(scenarioContext);
        }
        ...
        public async Task WhenGetUserById(string userId)
        {
            ...
            {
                TestLogger.AttachText(
                    _restClient.ResponseContent,
                    $"User_{userId}_Response.json");
            }
        }
        ...
        public async Task WhenGetAllUsers()
        {
            ...
            var users =
                await _restClient.GetAsync<List<UserModel>>("/users");
        ...
        {
            var response =
                _scenarioContext.Get<string>("LastResponse");
        ...
        {
            var user =
                _scenarioContext.Get<UserModel?>("LastUser");
        ...
        {
            var users =
                _scenarioContext.Get<List<UserModel>?>("UsersList");

            TestLogger.LogStep(
                "Verifying users list is present and not empty");
        ...
        public void ThenUserHasName(string expectedName)
        {
            ...
            TestLogger.LogStep(
                $"Verifying user name is '{expectedName}'");
        ...
        public void ThenUserHasEmail(string expectedEmail)
        {
            ...
            TestLogger.LogStep(
                $"Verifying user email is '{expectedEmail}'");
